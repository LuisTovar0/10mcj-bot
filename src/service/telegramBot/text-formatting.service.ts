...
interface AllInfo extends TextInfo {
  date: string;
}

export function inRequestsToString(reqs: NumberOfRequestsByUser) {
  let res = '';
  reqs.forEach(req => {
...
export function getDateStr() {
  const { day, month, year } = theDate();
  return `${day}-${(month < 10 ? `0` : ``) + month}-${year.toString().substring(2, 4)}`;
}

export function analyseMsgPT(text: string) {
...
export function telegramSignalPT({ date, descr1, descr2, url }: AllInfo) {
  const telegram = `${date}\n\n*${descr1}*${descr2 ? `\n_${descr2}_` : ``}\n\n[\u{25B6} YouTube](${url})\
...
export function getFullInfo(text: string) {
  const { url, descr1, descr2 } = analyseMsgPT(text);
  const date = getDateStr();
  const { telegram, signal } = telegramSignalPT({ date, url, descr1, descr2 });
